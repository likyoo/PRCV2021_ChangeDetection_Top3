...
from .decoder import UFPNDecoder
from ..encoders import get_encoder
from ..base import SegmentationModel
from ..base import SegmentationHead, ClassificationHead, SegmentationOCRHead
from change_detection_pytorch.base.heads import SegmentationCondHead


class UFPNnet(SegmentationModel):
    ...
    def __init__(
        ...
        if head == "seg":
            self.segmentation_head = nn.ModuleList([SegmentationHead(
            ...
            self.cond = True
            self.segmentation_head = nn.ModuleList([SegmentationCondHead(
                in_channels=decoder_channels[i],
                out_channels=classes,
                activation=activation,
            ) for i in loss_indexs])
        ...
        self.name = "ufpn-{}".format(encoder_name)
        self.initialize()
